...
const renderCartList = async () => {
  ...
  if (cartControl.cartData.length) {
    ...
    const cardsDataPromises = cartControl.cartData.map(
      async (item) =>
        await getData(
          `https://go-go-pizza-api.onrender.com/api/products/${item.id}`
        )
    );
    ...
    const cardsCart = cardsData.map((data, i) => {
      ...
      cardCart.innerHTML = `
        <picture>
          ...
          <img class="modal-cart__image" src="${
            data.images[0]
          }" width="63" height="63"
            alt="${data.name.ru}">
        </picture>
        ...
        <div class="modal-cart__content">
          <h3 class="modal-cart__title-pizza">${changeFirstUpperCase(
            data.name.ru
          )}</h3>
          <p class="modal-cart__info-pizza">
            <span class="modal-cart__price-pizza">${
              data.price[item.size]
            } ₽</span>
            <span>/</span>
            <span class="modal-cart__weight-pizza">${parseInt(
              item.size
            )} см</span>
          </p>
        </div>
...
const submitOrder = async (e) => {
  ...
  try {
    const response = await fetch(
      `https://go-go-pizza-api.onrender.com/api/orders`,
      {
        method: 'POST',
        headers: {
    ...
    `;
    cartPriceElem.forEach((elem) => (elem.textContent = ''));
